...
public class UserServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		case "VIEW":
			...
			User ul1 = dao.search(useq3);
			request.setAttribute("ul", ul1);
			request.getRequestDispatcher("WEB-INF/user/view.jsp").forward(request, response);	
			break;
		case "MODIFYVIEW":
			String useq4 = request.getParameter("useq");
			int useq5 = Integer.parseInt(useq4);
			User ul2 = dao.search(useq5);
			request.setAttribute("ul", ul2);
			request.getRequestDispatcher("WEB-INF/user/modify.jsp").forward(request, response);	
			break;
		case "MODIFYSAVE":
			String useq6 = request.getParameter("useq");
			int useq7 = Integer.parseInt(useq6);
			String name1 = request.getParameter("name");
			String email1 = request.getParameter("email");
			String phone1 = request.getParameter("phone");
			dao.update(useq7, name1, email1, phone1);
